...
- preserve secondary MIDI changes when primary senders repeat unchanged full-state snapshots
- identify ComfyUI WebSocket loader and output connections as persistent services

## [1.1.23 - 2026-06-25]

### Added

- add `MOODS_NAME_ONLY` output to `VrchAudioMusic2EmotionNode`
- add `TEXT Word Replacer @ vrch.ai` for rule-based text replacement
...
- process the latest `/image` websocket frame before falling back to cached image data
- show slow realtime image client warnings in ordinary websocket logs

## [1.1.21 - 2026-06-02]

### Updated

- update `VrchLiveConsoleControlNode` with `collapse_side_bar` sidebar control
...
- update `VrchLiveConsoleControlNode` to support `shaderEditor` pane visibility control and document the new `display_shader_editor` toggle
...
- add `VrchLiveConsoleControlNode` for WebSocket-driven live console control
- add `VrchWorkflowApiExportNode` (`Export Workflow API @ vrch.ai`) for server-side API workflow export

## 1.1.13 - 2026-02-21

### Updated

- Enhance WebSocket server and client functionality
